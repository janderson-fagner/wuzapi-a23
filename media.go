...
func (mycli *MyClient) processMedia(
	...
	// Local delivery serves the file over HTTP via ServeMedia, so it must
	// survive past this function; only schedule deletion for other modes.
	if s3cfg.MediaDelivery != "local" {
		defer func() {
	...
	if s3cfg.MediaDelivery == "local" {
		...
		postmap["mediaURL"] = baseURL + "/media/user_" + mycli.userID + "/" + fileName + "?token=" + url.QueryEscape(mycli.token)
